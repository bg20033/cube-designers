...
import {
  ...
  type RouteFilter,
  type RouteId,
} from "@/components/roadmap/useRoadmapController"
...
function PhaseDetails({
  ...
  return (
    <article className={mobile ? "rm2-mobile-card" : "rm2-inspector-card"}>
      ...
      <div
        className="rm2-card-section rm2-card-outputs"
        tabIndex={0}
        aria-label={`Outputs për fazën ${milestone.title}`}
      >
        <span>
          {activeRoute === "all"
...
export default function RoadmapView() {
  const {
    ...
    lineProgress,
    scrollToPhase,
  } = useRoadmapController(milestones)

  return (
    <section className="rm2-roadmap">
      <h1 className="rm2-sr-title">
        Roadmap 90-ditor: nga ideja te sistemi
      </h1>
      <section className="rm2-journey" ref={journeyRef}>
        <div className="rm2-sticky-stage">
          ...
          <div className="rm2-stage-workspace">
            ...
            <p className="rm2-sr-title" aria-live="polite" aria-atomic="true">
              Dita {currentDay}. Faza {milestones[activePhase].title}.{" "}
              {activeRoute === "all"
                ? "Sistemi i plotë"
                : routes.find((route) => route.id === activeRoute)?.label}
            </p>
